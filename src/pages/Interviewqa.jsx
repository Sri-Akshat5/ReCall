...
import SystemDesignInterview from "../components/systemDesign/SystemDesignInterview";
import {
  Code2,
  Layers,
  Cpu,
...
export const Interviewqa = ({ onSelectRecallCard, searchTerm }) => {
  ...
  const subjects = [
    ...
    {
      id: "systemdesign",
      name: "Distributed System Design Q&A",
      desc: "CAP & PACELC Theorem, Consistent Hashing, Database Sharding, LSM Trees, Redis Lua Atomicity, Saga Transactions & HLD Deep Dives.",
      icon: Cpu,
      questionCount: 55,
      active: true,
      badge: "55 Questions",
      tags: ["CAP Theorem", "Consistent Hashing", "Redis Lua", "Saga Pattern", "LSM Trees"]
    },
    {
      id: "react",
  ...
  // If System Design is selected, show SystemDesignInterview component
  if (activeSubject === "systemdesign") {
    return (
      <div className="space-y-6 font-sans">
        <div className="flex items-center justify-between">
          <button
          ...
          <div className="text-xs font-mono text-slate-500 dark:text-zinc-400">
            Subject: <span className="text-slate-900 dark:text-white font-bold font-mono">Distributed System Design Q&A</span>
          </div>
        </div>

        <SystemDesignInterview onSelectRecallCard={onSelectRecallCard} searchTerm={searchTerm} />
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {subjects.map((subject) => {
          ...
          return (
            <div
              key={subject.id}
              onClick={() => subject.active && handleSelectSubject(subject.id)}
                  ...
                  <div
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center font-bold ${
                      subject.active
                        ...
                        : "bg-slate-200 text-slate-500 dark:bg-zinc-900 dark:text-zinc-500"
                    }`}
                  >
                ...
                <div className="space-y-2">
                  <h2
                    className={`text-xl font-bold transition-colors ${
                      subject.active
                        ? "text-slate-900 dark:text-white group-hover:text-slate-700 dark:group-hover:text-zinc-300"
